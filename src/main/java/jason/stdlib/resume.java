 ...
 * 
 * <p>
 * Description: resume goals <i>G</i> that were suspended by <code>.suspend</code>.
 * <br/>
    ...
    public Object execute(TransitionSystem ts, Unifier un, Term[] args) throws Exception {
        ...
        while (ik.hasNext()) {
            ...
            if (i.isSuspended() && i.hasTrigger(g, un)) {
                ...
                boolean notify = true;
                if (k.startsWith(suspend.SUSPENDED_INT)) { // if not SUSPENDED_INT, it was suspended while already in PI, so, do not remove it from PI, just change the suspeded
                                                            // status
                    ik.remove();
