...
/**
 * <p>
 * Internal action: <b><code>.wait(<i>E</i>,<i>T</i>)</code></b>.
 * 
 * <p>
 * Description: suspend the intention for the time specified by <i>T</i> (in
    ...
    class WaitEvent implements CircumstanceListener {
        ...
        void resume(final boolean stopByTimeout) {
            ...
            ts.runAtBeginOfNextCycle(new Runnable() {
                public void run() {
                    try {
                        // add SI again in C.I if (1) it was not removed (2) is is not running (by some other reason) -- but this test does not apply to atomic intentions --, and
                        // (3) this wait was not dropped
