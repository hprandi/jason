 ...
 * 
 * <p>
 * Example in .mas2j project, the agent c uses a JDBC belief base:<br>
 * <br/>
 * 
 * <pre>
agents:
 ...
 * 
 * <p>
 * The predicate <code>a/1</code> is mapped to a table called "tablea" with an integer column called runs;
...
public class JDBCPersistentBB extends ChainBBAdapter {
	...
	protected boolean isCreatedByJason(PredicateIndicator pi) throws SQLException {
		...
		if (meta != null) {
			...
			return cols >= extraCols && meta.getColumnName((cols - extraCols) + 1).equalsIgnoreCase(COL_NEG)
    ...
    public static Structure timestamp2structure(Timestamp timestamp) throws SQLException {
        ...
        return ASSyntax.createStructure(timestampFunctor, createNumber(time.get(Calendar.YEAR)), createNumber(time.get(Calendar.MONTH)),
                createNumber(time.get(Calendar.DAY_OF_MONTH)), createNumber(time.get(Calendar.HOUR_OF_DAY)), createNumber(time.get(Calendar.MINUTE)),
    ...
    public static Timestamp structure2timestamp(Term timestamp) throws Exception {
        if (timestamp.isStructure()) {
            ...
            if (s.getFunctor().equals(timestampFunctor) && s.getArity() == 6) {
                return new Timestamp((int) ((NumberTerm) s.getTerm(0)).solve() - 1900, (int) ((NumberTerm) s.getTerm(1)).solve(), (int) ((NumberTerm) s.getTerm(2)).solve(),
                        (int) ((NumberTerm) s.getTerm(3)).solve(), (int) ((NumberTerm) s.getTerm(4)).solve(), (int) ((NumberTerm) s.getTerm(5)).solve(), 0);
            }
        }
        return null;
    }
