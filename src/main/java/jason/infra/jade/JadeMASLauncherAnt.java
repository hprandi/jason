    ...
    protected String replaceMarks(String script, boolean debug) {
		...
		for (String container : containers) {
			...
			if (container.equals("Main-Container")) {
				...
				if (!JadeAgArch.isCartagoJadeCase(project) && project.getEnvClass() != null
			...
			for (AgentParameters ap : project.getAgents()) {
				for (int cAg = 0; cAg < ap.getNbInstances(); cAg++) {
					...
					if (ap.getNbInstances() > 1)
						numberedAg += (cAg + 1);
					if ((container.equals("Main-Container") && ap.getHost() == null && allocator == null) || (ap.getHost() != null && ap.getHost().equals(container))
