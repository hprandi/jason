...
public class TransitionSystem {
        ...
        if (conf.C.hasMsg()) {
            ...
            if (intention != null) {
                ...
                if (!m.isReplyToSyncAsk()) { // ignore answer after the timeout
                    ...
                    if (!added) {
                        Literal received = new LiteralImpl(kqmlReceivedFunctor).addTerms(new Atom(sender), new Atom(m.getIlForce()), content, new Atom(m.getMsgId()));
    ...
    private void resumeSyncAskIntention(String msgId, Term answerVar, Term answerValue) throws JasonException {
        ...
        if (i.peek().getUnif().unifies(answerVar, answerValue)) {
            ...
            generateGoalDeletion(i, JasonException.createBasicErrorAnnots("ask_failed",
                    "reply of an ask message ('" + answerValue + "') does not unify with fourth argument of .send ('" + answerVar + "')"));
        }
        ...
        if (conf.C.SE.trigger.isGoal() && !conf.C.SE.trigger.isMetaEvent()) {
            ...
            try {
                if (conf.C.SE.getIntention() != null && conf.C.SE.getIntention().size() > 3000) {
                    logger.warning("we are likely in a problem with event " + conf.C.SE.getTrigger() + " the intention stack has already " + conf.C.SE.getIntention().size()
                            + " intended means!");
                }
    ...
    private void applySelInt() throws JasonException {
        ...
        // Rule SelInt1
        if (!conf.C.isAtomicIntentionSuspended() && conf.C.hasIntention()) { // the isAtomicIntentionSuspended is necessary because the atomic intention may be suspended (the above
                                                                                // removeAtomicInt returns null in that case)
    ...
    public void applyClrInt(Intention i) throws JasonException {
        while (true) { // quit the method by return
            ...
            if (im.getTrigger().isGoal() && !im.getTrigger().isAddition() && !i.isFinished()) {
                ...
                // +!s: !z
                im = i.peek();
                if (im.isFinished() || !(im.unif.unifies(im.getCurrentStep().getBodyTerm(), topLiteral) && im.getCurrentStep().getBodyType() == BodyType.achieve)
                        || im.getCurrentStep().getBodyTerm() instanceof VarTerm) {
                    im = i.pop(); // +!c above
                }
                while (!i.isFinished() && !(im.unif.unifies(im.getTrigger().getLiteral(), topLiteral) && im.getTrigger().isGoal())
    ...
    public boolean canSleep() {
        return (C.isAtomicIntentionSuspended() && !C.hasFeedbackAction() && !conf.C.hasMsg()) // atomic case
                || (!conf.C.hasEvent() && // other cases (deliberate)
                        ...
                        !conf.C.hasMsg() && // (sense)
                        taskForBeginOfCycle.isEmpty() && getUserAgArch().canSleep());
    }
    ...
    public void deliberate() {
        try {
            ...
            while (r != null) {
                r.run(); // It is processed only things related to operations on goals/intentions resumed/suspended/finished It can be placed in the deliberate stage, but the
